...
import org.uwtech.epix.epixuhc.events.UhcWinEvent;
...
import org.uwtech.epix.epixuhc.threads.CheckRemainingPlayerThread;
import org.uwtech.epix.epixuhc.threads.TeleportPlayersThread;
import org.bukkit.*;
...
import org.uwtech.epix.epixuhc.threads.UpdateScoreboardThread;
...
public class PlayersManager {
		...
		switch(uhcGM.getGameState()){
			...
			case PLAYING:
					try{
						...
						if(player.hasPermission("epixuhc.join-override") || uhcGM.getConfiguration().getCanJoinAsSpectator() && uhcGM.getConfiguration().getCanSpectateAfterDeath()){
			...
			case ENDED:
				if(player.hasPermission("epixuhc.join-override")){
					return true;
				}
	...
	public void setPlayerStartPlaying(UhcPlayer uhcPlayer){
		...
		if(!uhcPlayer.getHasBeenTeleportedToLocation()){
			...
			Bukkit.getScheduler().runTaskLater(EpixUHC.getPlugin(), new UpdateScoreboardThread(uhcPlayer), 10);
	...
	public void randomTeleportTeams() {
		...
		Bukkit.getScheduler().runTaskLater(EpixUHC.getPlugin(), () -> GameManager.getGameManager().startWatchingEndOfGame(), delayTeleportByTeam + 20);
	...
	public void startWatchPlayerPlayingThread() {
		...
		Bukkit.getScheduler().runTaskLaterAsynchronously(EpixUHC.getPlugin(), new CheckRemainingPlayerThread() , 40);
	}
