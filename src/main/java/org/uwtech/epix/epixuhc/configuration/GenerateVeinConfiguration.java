...
public class GenerateVeinConfiguration {
	...
	public boolean parseConfiguration(ConfigurationSection section){
		try{
			...
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse section '"+section.getName()+"' in generate-vein. This is not an existing block type.Ignoring it.");
			return false;
		}
		...
		if(minVeinsPerChunk < 0 || maxVeinsPerChunk < 0){
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse section '"+section.getName()+"' in generate-vein. min and max-veins-per-chunk must be positive.");
			return false;
		}
		...
		if(minBlocksPerVein < 0 || maxBlocksPerVein < 0){
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse section '"+section.getName()+"' in generate-vein. min and max-blocks-per-vein must be positive.");
			return false;
		}
		...
		if(minY < 0 || minY > 255 || maxY < 0 || maxY > 255){
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse section '"+section.getName()+"' in generate-vein. The min and max Y must be between 0 and 255.");
			return false;
		}
