...
public class BlockLootConfiguration {
	...
	public boolean parseConfiguration(ConfigurationSection section){
		try{
			...
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse section '"+section.getName()+"' in block-loot. This is not an existing block type. Ignoring it.");
			return false;
		}
		...
		try{
			...
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse loot '"+material.toString()+"' in block-loot. The syntax must be 'MATERIAL/QUANTITY/DAMAGE'.Ignoring it.");
			return false;
		}
