...
public class MobLootConfiguration {
	...
	public boolean parseConfiguration(ConfigurationSection section){
		try{
			...
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse section '"+section.getName()+"' in mob-loot. This is not an existing entity type. Ignoring it.");
			return false;
		}
		...
		try{
			...
			Bukkit.getLogger().warning("[EpixUHC] Couldn't parse loot '"+entity.toString()+"' in mob-loot. The syntax must be 'MATERIAL/QUANTITY/DAMAGE'.Ignoring it.");
			return false;
		}
