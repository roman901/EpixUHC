package org.uwtech.epix.epixuhc.maploader;

import org.uwtech.epix.epixuhc.EpixUHC;
import org.uwtech.epix.epixuhc.game.GameManager;
import org.uwtech.epix.epixuhc.game.UhcWorldBorder;
...
public class MapLoader {
	...
	public void createNewWorld(Environment env){
		...
		if(env.equals(Environment.NORMAL)){
			EpixUHC.getPlugin().getConfig().set("worlds.overworld", uuid.toString());
			...
			EpixUHC.getPlugin().getConfig().set("worlds.nether", uuid.toString());
			gm.getConfiguration().setNetherUuid(uuid.toString());
		}		
		EpixUHC.getPlugin().saveConfig();
	...
	public void generateChunks(final Environment env){
		...
		Bukkit.getScheduler().runTaskAsynchronously(EpixUHC.getPlugin(), new Runnable(){
			...
			public void run() {				
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(EpixUHC.getPlugin(), new RunnableWithParameter(-maxChunk,-maxChunk,restEveryTicks),0);
